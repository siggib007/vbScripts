...
Const ReportTitle = "IHS Device Reachability Report"
...
Dim arg1, arg2
	...
	strout = strout & "<br>Further details available at "
	strout = strout & "<a href=""http://ineteng/net/simpleQuery.aspx""> http://ineteng/net/simpleQuery.aspx.</a>.<br>" & vbcrlf
	strout = strout & "The ""Internet Device - Failed Test"" query will show this report."
	strout = strout & "<h3><br></h3>" & vbcrlf

	wscript.echo "Creating Table"
	'Call the DB for DC Overview data
	cmdText = "select * from vwReachability order by devicename"
    rs.Open cmdText, cn
        
    'Create HTML Table from the results
	ConvertRS2HTMLTable "Devices that failed reachability test"
	rs.close
