...
Dim objfileout, sOutPath, errmsg, today
...
today = DatePart("YYYY",Now) & Right("0" & DatePart("m",Now),2) & Right("0" & DatePart("d",Now),2)
...
Do until maxdate > Now()
	If fso.fileexists(sfilepath) Then
		...
		'wscript.echo "outfile " & soutpath & "\" & strdate & ".txt created."	
		'If strdate = today Then
		'	wscript.echo "Making a copy of today's syslog file."' Current file " & sfilepath & " dest: " & soutpath
			'fso.copyfile sfilepath, soutpath
		'	Set FileObj = fso.opentextfile(soutpath & "\" & strdate & ".log")
		'Else
		'	Set FileObj = fso.opentextfile(sfilepath)
		'End If
		'If strdate = today Then
		'	cmdtext = "delete from " & dbname & ".dbo" & tablename & " where dtTimeStamp > convert(datetime,convert(varchar(25),dateadd(day, -1,getdate()),1))" 
		'	Cmd.CommandText=cmdtext
		'	On Error Resume Next
		'	Cmd.Execute
		'	checkerror "Error while deleting today's results."
		'	On Error GoTo 0						
		'End If		
		Set FileObj = fso.opentextfile(sfilepath)
		...
		Do until fileobj.atendofstream
			...
			If UBound(strparts) > 3 Then
				...
				If dtline > DateAdd("d", 0, maxdate) Then
					device = strparts(4) 
					deviceIP = strparts(5)
					Severity = strparts(7)
					message = ""
					...
					End If
					On Error Resume Next
					objfileout.writeline dtline & vbtab & device & vbtab & deviceip & vbtab & severity & vbtab & strclass & vbtab & errmsg & vbtab & message
					checkerror "Error while writing outputline."
					On Error GoTo 0			
					linecount = linecount + 1
					If (linecount Mod 100000) = 0 and strdate = today Then 
		...
		checkerror "Error while bulk importing " & soutpath & "\" & strdate & ".txt."
		fso.deletefile soutpath & "\" & strdate & ".txt", true
		'checkerror "Error while deleting " & soutpath & "\" & strdate & ".txt."
		'If fso.fileexists(soutpath & "\" & strdate & ".log") Then
		'	fso.deletefile soutpath & "\" & strdate & ".log", true
		'End If
		'checkerror "Error while deleting " & soutpath & "\" & strdate & ".log."
		On Error GoTo 0			
