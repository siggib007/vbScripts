...
Sub Main
	...
	dim strParts, strLine, objFileIn, objFileOut, objFileVlan, host, ConCmd, fso, nError, strErr, strIPAddr, strAvailTrack, strVersion
	dim strOut, strOutPath, iResponse, strLineParts, strCommand, strComment, strTemp, bCont, strVlan, strTrack, strVlanComment, strType
	dim bTrack, bLo101, iVlanCount, strTempParts, objDebugOut, strUID, strPWD, strHSRPCmd
	...
	objFileOut.writeline "primaryIPAddress,hostName,AvailTrack,Lo101,SVIcount,comment,OS"
	...
	While not objFileIn.atendofstream
		...
		If crt.Session.Connected Then
			...
			if instr(strVersion,"Cisco IOS")>0 Then
				strHSRPCmd = "show standby brief"
				strType = "IOS"
			end if
			if instr(strVersion,"Cisco Nexus")>0 Then
				strHSRPCmd = "show hsrp brief"
				strType = "Nexus"
			end if
			...
			' crt.Screen.WaitForString "#",Timeout, True
			if strHSRPCmd <> "" then
				strCommand = strHSRPCmd
				bCont=True
			else
				bCont = False
				objDebugOut.writeline "Unknown device " & strVersion
				strComment = strComment & "Don't have HSRP command for this device;"
			end if
			crt.Screen.Send(strCommand & vbcr)
			do while bcont
			...
			next
			objFileOut.writeline strIPAddr & "," & host & "," & strAvailTrack & "," & bLo101 & ","	& iVlanCount & "," & strComment & "," & strType
		else
