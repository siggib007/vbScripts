Option Explicit 
Dim cn, fso, strOutFile, cmd, strAttach(), i
...
Const Indent = "    "
Const DBServer = "sgbt9"
Const DefaultDB = "test"
Const UserID = ""
Const PWD = ""
Const cmdText1 = "spPortReport"
Const ReportTitle = "Canyon Park Port Report"

ReDim strAttach(0)
strattach(0)=""
'strAttach(0)="\\tkihssql01\c$\sqltemp\BillingSideBySideComparison.xls"
'strAttach(1)="\\tkihssql01\c$\sqltemp\ServerList.xls"
...
Dim arg1, arg2, arg3, ians
	...
	If strOutFilePath = "" and ToAddress="" Then
		...
		wscript.echo " "
		wscript.echo "cscript " & wscript.scriptname & " [email | filePath [/y]]"
		wscript.echo " "
	...
	While not rs.eof
	  For x=0 to rs.fields.count - 1
	  	Select Case rs.fields(x).type
	  		...
	  		Case 4,5,131 ' Numeric
		  		'wscript.echo "Field: " & rs.fields(x).name & " is a float. Value=" & rs.fields(x).value
	  			If not IsNull(rs.fields(x).value) Then 
			  		fldValue = FormatPercent (CDbl(rs.fields(x).value),2,TristateTrue,TristateFalse,TristateTrue)
	  				'wscript.echo "Field: " & rs.fields(x).name & " is an int. Value=" & rs.fields(x).value
	  			Else
...
Sub MySendMail(StrSubject,msg)
	...
	With iMsg 
	  Set .Configuration = iConf 
	      ...
	      .Subject  = StrSubject 
	      .HTMLBody = msg
	      For i = 0 to UBound(strAttach)
	      	If strattach(i)="" Then Exit For
	      	.AddAttachment(strAttach(i))
	      Next
	      .Send 
	End With
End Sub
